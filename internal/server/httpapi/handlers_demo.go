...
func (s *Server) handleDemoRuns(w http.ResponseWriter, r *http.Request) {
	...
	type demoResponse struct {
		core.DemoRun
		Links core.RunLinks `json:"links"`
	}
	response := struct {
...
func (s *Server) serveDemoReport(w http.ResponseWriter, r *http.Request, demo core.DemoRun) {
	if r.Method != http.MethodGet {
		writeError(w, r, http.StatusMethodNotAllowed, "method_not_allowed", "method not allowed", nil)
		return
	}
...
func (s *Server) serveDemoArtifact(w http.ResponseWriter, r *http.Request, demo core.DemoRun, artifactPath string) {
	if r.Method != http.MethodGet {
		writeError(w, r, http.StatusMethodNotAllowed, "method_not_allowed", "method not allowed", nil)
		return
	}
...
func (s *Server) loadDemoIndex() (core.DemoIndex, error) {
	var index core.DemoIndex
	b, err := os.ReadFile(s.Config.DemoIndexPath)
	if err != nil {
		return core.DemoIndex{}, err
	}
...
func (s *Server) findDemo(id string) (core.DemoRun, bool, error) {
	index, err := s.loadDemoIndex()
	if err != nil {
		return core.DemoRun{}, false, err
	}
	for _, demo := range index.DemoRuns {
		if demo.ID == id {
