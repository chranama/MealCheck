...
type PendingInputs = runinput.Vault
type CleanupJob = execution.CleanupJob
type reviewCorrectionRequest = review.Correction
...
func NewPendingInputs() *runinput.Vault {
	return runinput.New()
}

func NewWorker(config Config, store state.Store, inputs *runinput.Vault, factory inference.CompleterFactory) *execution.Worker {
	return execution.NewWorker(config, store, inputs, factory)
}

func newRun(config Config, casePath string) Run {
	return submission.NewRun(config, casePath, time.Now().UTC())
}

func runProgress(run Run, events []RunEvent) *RunProgress {
	return progress.Run(run, events)
}
