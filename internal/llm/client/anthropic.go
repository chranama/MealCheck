...
func anthropicMessagePayload(messages []Message) (string, []map[string]string) {
...
func (p Anthropic) Complete(ctx context.Context, config ProviderConfig, request Request) (string, error) {
	if config.APIKey == "" {
	...
	system, anthropicMessages := anthropicMessagePayload(request.Messages)
	...
	if request.StructuredOutput != nil {
		schema := request.StructuredOutput.PortableSchema
		if schema == nil {
