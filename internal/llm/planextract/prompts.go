...
func localModelExtractionMessagesForMealChunk(input PendingRunInput, chunk localLlamaMealChunk) ([]Message, error) {
	...
	userParts := []string{
		...
		"Use context-only meal text only to clarify the authoritative source items; never extract rows from context text.",
		"Do not add foods from context-only meal text unless they appear in the authoritative source items.",
		...
		"Source_text overrides meal-text wording and order.",
