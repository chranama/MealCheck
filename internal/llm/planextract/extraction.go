...
// Extract normalizes hosted meal-plan text through the same chunked extraction
// path used by live run creation.
func Extract(ctx context.Context, completer Completer, providerConfig ProviderConfig, input PendingRunInput, planID string) (string, checker.Plan, []LocalLlamaNormalizationRepair, string, error) {
	output, plan, repairs, _, stage, err := extractWithArtifacts(ctx, completer, providerConfig, input, planID)
	return output, plan, repairs, string(stage), err
}

// ExtractWithArtifacts normalizes hosted meal-plan text and returns the
// chunk-level evidence artifact captured during extraction.
func ExtractWithArtifacts(ctx context.Context, completer Completer, providerConfig ProviderConfig, input PendingRunInput, planID string) (string, checker.Plan, []LocalLlamaNormalizationRepair, *LocalModelExtractionArtifact, string, error) {
	output, plan, repairs, extraction, stage, err := extractWithArtifacts(ctx, completer, providerConfig, input, planID)
	return output, plan, repairs, extraction, string(stage), err
}

func extract(ctx context.Context, completer Completer, providerConfig ProviderConfig, input PendingRunInput, planID string) (string, checker.Plan, []LocalLlamaNormalizationRepair, localModelExtractionFailureStage, error) {
	output, plan, repairs, _, stage, err := extractWithArtifacts(ctx, completer, providerConfig, input, planID)
	return output, plan, repairs, stage, err
}

func extractWithArtifacts(ctx context.Context, completer Completer, providerConfig ProviderConfig, input PendingRunInput, planID string) (string, checker.Plan, []LocalLlamaNormalizationRepair, *LocalModelExtractionArtifact, localModelExtractionFailureStage, error) {
	started := time.Now()
	...
	for index, chunk := range chunks {
		...
		output, err := completer.Complete(ctx, providerConfig, Request{
			Messages: messages,
			StructuredOutput: &StructuredOutput{
				Name:         "mealcheck_compact_meal_plan",
				StrictSchema: MealChunkResponseSchema(),
			},
		})
