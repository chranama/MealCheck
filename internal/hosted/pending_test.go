...
func TestPendingInputsTakeDeletesAndRejectsExpiredInputs(t *testing.T) {
	...
	pending.Put("fresh", PendingRunInput{Mode: "profile_generation"}, now.Add(time.Minute))
	...
	pending.Put("expired", PendingRunInput{Mode: "prompt_generation"}, now.Add(-time.Second))
