...
func TestStoreClaimBlocksSecondLocalModelRun(t *testing.T) {
	ctx := context.Background()
	store := New()
	now := time.Date(2026, 7, 1, 12, 0, 0, 0, time.UTC)

	first := testRun("run_local_1", state.InputModeLocalModel, now)
	second := testRun("run_local_2", state.InputModeLocalModel, now.Add(time.Second))
...
func TestStoreClaimAllowsNonLocalRunWhileLocalModelRuns(t *testing.T) {
	ctx := context.Background()
	store := New()
	now := time.Date(2026, 7, 1, 12, 0, 0, 0, time.UTC)

	localRunning := testRun("run_local_1", state.InputModeLocalModel, now)
	localQueued := testRun("run_local_2", state.InputModeLocalModel, now.Add(time.Second))
	byokQueued := testRun("run_byok", "prompt_generation", now.Add(2*time.Second))
	for _, run := range []state.Run{localRunning, localQueued, byokQueued} {
	...
	if local.Status != state.StatusQueued {
		t.Fatalf("queued local status = %q, want queued", local.Status)
	}
}

func testRun(id, inputMode string, createdAt time.Time) state.Run {
	return state.Run{
		ID:          id,
