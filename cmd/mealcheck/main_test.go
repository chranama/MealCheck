...
func TestEvalNormalizationCommandWritesResult(t *testing.T) {
	...
	if result.TotalCases != 14 || result.SuccessCases != 11 || result.FailureCases != 3 {
		t.Fatalf("case counts = total %d success %d failure %d, want 14/11/3", result.TotalCases, result.SuccessCases, result.FailureCases)
	}
	if result.CasesWithMismatches != 0 {
		t.Fatalf("cases_with_mismatches = %d, want 0", result.CasesWithMismatches)
	}
